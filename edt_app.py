...
with st.sidebar:
    ...
    st.markdown("---")
    mode_view = st.sidebar.radio("Vue :", ["Promotion", "Enseignant", "🚩 Vérificateur", "🔍 Salles & Amphis Libres"])
...
if df is not None:
    ...
    try:
        if mode_view == "🔍 Salles & Amphis Libres":
            st.subheader("🏢 Salles et Amphis non occupés")
            # On ne garde que les noms contenant 'Salle', 'Amphi', 'S' ou 'A'
            mots_cles = ["SALLE", "AMPHI", "S", "A"]
            tous_locaux = sorted([l for l in df['Lieu'].unique() if l not in ["Non défini", ""] and any(m in l.upper() for m in mots_cles)])
