...
st.markdown("""
    ...
    .salle-libre { color: #28a745; font-size: 10px; font-weight: bold; }
    ...
    try:
        if mode_view == "🚩 Vérificateur":
            ...
            def get_lieux_libres(group):
                ...
                return "<br>".join([f"<span class='salle-libre'>🟢 {l}</span>" for l in libres])
            ...
            def get_type(t):
                t = t.upper()
                if "COURS" in t: return "COURS"; elif "TD" in t: return "TD"; elif "TP" in t: return "TP"
                return "AUTRE"
            ...
            charge_reelle, charge_reg = df_stats['h_val'].sum(), (3.0 if poste_superieur else 6.0)
            h_sup = charge_reelle - charge_reg
            n_c, n_td, n_tp = len(df_stats[df_stats['Type'] == "COURS"]), len(df_stats[df_stats['Type'] == "TD"]), len(df_stats[df_stats['Type'] == "TP"])
