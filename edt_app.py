...
st.markdown(f"""
    ...
    .conflict {{ background-color: #ffcccc !important; color: #b71c1c; font-weight: bold; border: 2px solid red !important; }}
    .separator {{ border-top: 1px dashed #bbb; margin: 4px 0; }}
    @media print {{ section[data-testid="stSidebar"], button {{ display: none !important; }} }}
    </style>
""", unsafe_allow_html=True)

# --- VARIABLES GLOBALES (FIX NameError) ---
jours_list = ["Dimanche", "Lundi", "Mardi", "Mercredi", "Jeudi"]
heures_list = ["8h - 9h30", "9h30 - 11h", "11h - 12h30", "12h30 - 14h", "14h - 15h30", "15h30 - 17h"]
...
if not st.session_state["user_data"]:
    ...
    with t2:
        ...
        new_nom = st.selectbox("Choisir votre nom", sorted(df['Enseignants'].unique()))
        ...
        if st.button("Valider l'inscription"):
            ...
            except: st.error("Erreur (Email déjà pris).")
    with t3:
        if st.text_input("Code Admin", type="password") == "doctorat2026":
            if st.button("Accéder"): st.session_state["user_data"] = {"nom_officiel": "ADMIN", "role": "admin"}; st.rerun()
    st.stop()
...
with st.sidebar:
    ...
    mode = st.radio("Menu", ["Promotion", "Enseignant", "🚩 Chevauchements"]) if is_admin else "Personnel"
...
if mode == "Personnel" or (is_admin and mode == "Enseignant"):
    cible = user['nom_officiel'] if mode == "Personnel" else st.selectbox("Choisir Enseignant", sorted(df['Enseignants'].unique()))
    ...
    # Tableau Enseignant (avec Lieux et Conflits)
    def fmt_ens(rows):
        res = []
        for _, r in rows.iterrows():
            cl = "class='conflict'" if r['Conflict'] else ""
            res.append(f"<div {cl}><b>{r['Enseignements']}</b><br>({r['Promotion']})<br>📍 <i>{r['Lieu']}</i></div>")
        return "<div class='separator'></div>".join(res)

    grid = df_f.groupby(['Horaire', 'Jours']).apply(fmt_ens).unstack('Jours').reindex(index=heures_list, columns=jours_list).fillna("")
    st.write(grid.to_html(escape=False), unsafe_allow_html=True)
    components.html("<button onclick='window.parent.print()' style='width:100%; padding:10px; background:#28a745; color:white; border:none; border-radius:5px; font-weight:bold; cursor:pointer; margin-top:20px;'>🖨️ IMPRIMER L'EMPLOI DU TEMPS</button>", height=70)

elif mode == "Promotion" and is_admin:
    p_sel = st.selectbox("Choisir la Promotion", sorted(df['Promotion'].unique()))
    df_p = df[df['Promotion'] == p_sel]
    def fmt_p(rows):
        return "<br>".join([f"<b>{r['Enseignements']}</b><br>{r['Enseignants']}<br>📍 {r['Lieu']}" for _, r in rows.iterrows()])
    grid_p = df_p.groupby(['Horaire', 'Jours']).apply(fmt_p).unstack('Jours').reindex(index=heures_list, columns=jours_list).fillna("")
    st.write(f"### Planning Promotion : {p_sel}")
    st.write(grid_p.to_html(escape=False), unsafe_allow_html=True)

elif mode == "🚩 Chevauchements" and is_admin:
    st.subheader("🚩 Enseignants ayant des séances simultanées")
    conflits = df[df['Conflict'] == True]
    if conflits.empty: st.success("✅ Aucun chevauchement détecté.")
