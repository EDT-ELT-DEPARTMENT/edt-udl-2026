...
# Titre officiel rappelé à chaque exécution
...
def load_data():
    ...
    df_s = pd.read_excel(FICHICHIER_ETUDIANTS)
    df_staff = pd.read_excel(FICHIER_STAFF)
    # Nettoyage rigoureux des espaces dans les colonnes et les données
...
if not st.session_state["user_data"]:
    ...
    t_login, t_signup = st.tabs(["🔐 Connexion Enseignant", "📝 Inscription (Nouveau)"])
    ...
    with t_signup:
        st.info("Sélectionnez votre nom pour extraire vos données du fichier Staff.")
        ...
        st.success(f"Détecté : {info['Qualité']} | Grade : {info['Grade']}")
...
with st.sidebar:
    ...
    st.markdown(f"🎓 **Grade :** {user.get('grade_enseignant', 'Non renseigné')}")
...
with tab_saisie:
    ...
    c1, c2 = st.columns(2)
    # Filtrage intelligent des promotions selon l'enseignant sélectionné
    ...
    matiere_sel = c2.selectbox("📖 Matière :", list_matieres if list_matieres else ["Sélectionnez une promotion"])
    ...
    if not res_s.empty:
        ...
        st.warning("⚠️ Aucune correspondance trouvée dans l'EDT pour cette matière.")
    ...
    if abs_collective:
        ...
        etudiant_note = st.selectbox("Étudiant à noter (Participation/Quiz) :", ["Aucun"] + df_f['Full'].tolist())
        note_val = st.text_input("Note/Bonus :", value="0")

    obs = st.text_area("🗒️ Observations (Motif, Avancement...) :")
            ...
            with st.spinner("Archivage en cours..."):
                try:
                    ...
                    <div style="font-family: Arial; border: 2px solid #003366; padding: 20px; border-radius: 10px;">
                        ...
                        <p><b>Enseignant :</b> {enseignant_sel} ({user.get('grade_enseignant')})</p>
                        <p><b>Qualité :</b> {user.get('statut_enseignant')} | <b>Régime :</b> {regime_heure}</p>
                        <hr>
                        <p><b>Matière :</b> {matiere_sel} | <b>Promo :</b> {promo_sel}</p>
                        <p><b>Date :</b> {date_seance} | <b>Lieu :</b> {lieu_v} | <b>Horaire :</b> {horaire_v}</p>
                        <p><b>Absents :</b> {len(absents)} | <b>Note Participation :</b> {note_val}</p>
                        <p><b>Observations :</b> {obs}</p>
                        <p style="text-align: right;"><b>Signé :</b> {sign}</p>
                    </div>
                    """
...
with tab_hist:
    ...
    if res.data:
        ...
        st.download_button("📊 Exporter vers Excel", buf.getvalue(), "Archives_UDL_S2_2026.xlsx")
    else:
