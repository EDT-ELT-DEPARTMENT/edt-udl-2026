...
if not st.session_state["user_data"]:
    ...
    with t_forgot:
        ...
        if st.button("Générer nouveau code"):
            ...
            if res.data:
                ...
                send_email_rapport([f_email], "Réinitialisation Code UDL", f"Votre nouveau code : {new_c}")
                st.success("Envoyé par email.")
    ...
    with t_student:
        ...
        if nom_st != "--":
            ...
            # ... (Logique EDT déjà existante et fonctionnelle)
    st.stop()
...
with st.sidebar:
    ...
    st.divider()
    ens_actif = st.selectbox("Simuler :", sorted(df_edt['Enseignants'].unique())) if is_admin else user['nom_officiel']
...
with t_saisie:
    ...
    type_seance = c1.selectbox("Type :", ["Cours", "TD", "TP", "Examen"])
    ...
    abs_collective = st.checkbox("🚩 Absence Collective (Promotion/Groupe)")
    if abs_collective:
    ...
    obs_gen = st.text_area("🗒️ Observations générales de la séance :")
        ...
        if hash_pw(code_v) == user['password_hash']:
            ...
            for name in absents_final:
                supabase.table("archives_absences").insert({
                    ...
                    "observations": obs_gen, "categorie_seance": charge, "type_seance": type_seance
                }).execute()
            # Archivage Note
            ...
            send_email_rapport([EMAIL_CHEF_DEPT, EMAIL_ADJOINT], f"Rapport {m_sel}", f"Enseignant: {user['nom_officiel']}\nDate: {date_s}\nAbsents: {len(absents_final)}")
            st.success("Rapport enregistré !"); st.balloons()
        else: st.error("Code incorrect.")

# --- ONGLET 2 : SUIVI ÉTUDIANT (NOUVELLE LOGIQUE PRO) ---
with t_suivi:
    st.markdown("### 🔍 Fiche de Suivi Individuelle")
    
    # 1. Sélection Promotion (Filtre Enseignant)
    ...
    promo_suivi = st.selectbox("1️⃣ Choisir la Promotion :", mes_promos, key="suivi_p")
    
    # 2. Sélection Étudiant (Filtré par la promo choisie)
    etudiants_promo = df_etudiants[df_etudiants['Promotion'] == promo_suivi]
    nom_suivi = st.selectbox("2️⃣ Sélectionner l'Étudiant :", ["--"] + sorted(etudiants_promo['Full_N'].unique()), key="suivi_n")
    ...
    if nom_suivi != "--":
        ...
        if res.data:
            ...
            # On filtre ce qui contient "Absence" ou "Exclusion" dans note_evaluation
            df_abs = df_res[df_res['note_evaluation'].str.contains("Absence|Exclusion", na=False)]
            if not df_abs.empty:
                st.table(df_abs[['date_seance', 'matiere', 'note_evaluation', 'categorie_seance']])
            else:
                st.info("Aucune absence enregistrée.")

            # --- BLOC NOTATION ---
            st.markdown("##### 📝 Évaluations & Participation")
            # On filtre les lignes qui contiennent un critère de note (Test, Examen, Participation)
            df_notes = df_res[df_res['note_evaluation'].str.contains("Test|Examen|Participation|Interrogation", na=False)]
            if not df_notes.empty:
                st.table(df_notes[['date_seance', 'matiere', 'note_evaluation', 'type_seance']])
            else:
                st.info("Aucune note enregistrée.")

            # --- BLOC OBSERVATIONS ---
            st.markdown("##### 🗒️ Observations Générales Saisies")
            df_obs = df_res[df_res['observations'] != ""]
            if not df_obs.empty:
                for idx, row in df_obs.iterrows():
                    with st.expander(f"Séance du {row['date_seance']} - {row['matiere']}"):
                        st.write(f"**Observation :** {row['observations']}")
                        st.caption(f"Saisie par : {row['enseignant']}")
        else:
            st.warning("Aucune donnée trouvée en base pour cet étudiant.")
