...
def load_data():
    try:
        ...
        df_s = pd.read_excel(FICHICHIER_ETUDIANTS if 'FICHICHIER_ETUDIANTS' in locals() else FICHIER_ETUDIANTS)
...
with st.sidebar:
    ...
    if is_admin:
        ...
        enseignant_sel = st.selectbox("Vue Admin - Enseignant :", sorted(df_edt['Enseignants'].unique()), index=0)
    else:
...
with tab_saisie:
    ...
    c1, c2 = st.columns(2)
    mask = df_edt['Enseignants'].str.contains(enseignant_sel, na=False)
    ...
    list_matieres = sorted(df_edt[mask & (df_edt['Promotion'] == promo_sel)]['Enseignements'].unique())
...
# --- ONGLET 2 : ARCHIVES (CORRIGÉ POUR KEYERROR) ---
with tab_hist:
    ...
    if res.data:
        ...
        # SÉCURITÉ : On ne garde que les colonnes qui existent vraiment dans le DataFrame
        cols_souhaitees = ['date_seance', 'enseignant', 'statut_enseignant', 'matiere', 'regime_heure', 'etudiant_nom', 'note_evaluation']
        cols_existantes = [c for c in cols_souhaitees if c in df_arc.columns]
        
        st.dataframe(df_arc[cols_existantes], use_container_width=True)
        
        buf = io.BytesIO()
        df_arc.to_excel(buf, index=False)
        st.download_button("📊 Télécharger Excel", buf.getvalue(), "Archives_UDL.xlsx")
    else:
