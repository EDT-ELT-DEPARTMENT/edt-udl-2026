...
def send_notification_admin(details):
    """Envoi automatique au Chef de Dept et à l'Adjoint lors d'une saisie"""
...
def safe_insert(table_name, data_dict):
    """Insère les données en ignorant les colonnes qui feraient planter l'API"""
    try:
        return supabase.table(table_name).insert(data_dict).execute()
    except:
        # Liste restreinte de colonnes sûres (colonnes historiques de votre base)
...
# --- ONGLET 2 : SUIVI INDIVIDUEL (CORRECTION KEYERROR) ---
with tab_suivi:
    ...
    if et_q != "--":
        ...
        if r.data:
            ...
            # Sécurité : On ne garde que les colonnes qui existent vraiment dans df_r
...
# --- ONGLET 3 : ARCHIVE GLOBALE (PILOTAGE) ---
with tab_hist:
    ...
    if res_all.data:
        ...
        c_a.metric("Total Séances", len(df_disp['date_seance'].unique()))
        c_b.metric("Total Absences", len(df_disp[df_disp['note_evaluation'] == "ABSENCE"]))
        
        buf = io.BytesIO(); df_disp.to_excel(buf, index=False)
        st.download_button("📥 Télécharger cette vue (Excel)", buf.getvalue(), "Archives_Filtrées.xlsx")
